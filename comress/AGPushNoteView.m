...
+ (void)showWithNotificationMessage:(NSString *)message {
    ...
    [AGPushNoteView showWithNotificationMessage:[NSString stringWithFormat:@"%@ %@",message,[formatter stringFromDate:[NSDate date]]] completion:^{
        //Nothing.
    }];
...
+ (void)showWithNotificationMessage:(NSString *)message completion:(void (^)(void))completion {
    ...
    PUSH_VIEW.currentMessage = [NSString stringWithFormat:@"%@ %@",message,[formatter stringFromDate:[NSDate date]]];
