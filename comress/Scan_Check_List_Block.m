...
@implementation Scan_Check_List_Block
...
- (BOOL)updateLastRequestDateWithDate:(NSString *)dateString
{
    ...
    [myDatabase.databaseQ inTransaction:^(FMDatabase *theDb, BOOL *rollback) {
        FMResultSet *rs = [theDb executeQuery:@"select * from ro_scanchecklist_blk_last_req_date"];
        
        if(![rs next])
        {
            BOOL qIns = [theDb executeUpdate:@"insert into ro_scanchecklist_blk_last_req_date(date) values(?)",date];
        ...
        {
            BOOL qUp = [theDb executeUpdate:@"update ro_scanchecklist_blk_last_req_date set date = ? ",date];
